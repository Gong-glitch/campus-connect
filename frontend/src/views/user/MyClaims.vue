...
<template>
  ...
  <main class="mx-auto max-w-7xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
    ...
    <div v-if="isLoading" class="text-center py-12 rounded-md border bg-white shadow-soft">
      <p class="text-lg font-medium text-muted animate-pulse">Syncing live claims from server...</p>
    </div>

    <div v-else-if="fetchError" class="p-4 bg-danger/10 text-danger rounded-md text-sm font-medium border border-danger/20 text-center">
      {{ fetchError }}
    </div>
