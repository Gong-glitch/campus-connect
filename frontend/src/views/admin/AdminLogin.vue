...
onMounted(async () => {
  try {
    ...
    // 🚀 FIXED: Captures network failure instead of swallowing it silently
    console.error("Backend connection error:", err);
    error.value = "Cannot connect to server. Please check your backend connection.";
  }
});
    ...
    <form class="w-full max-w-md rounded-md border-t-4 border-primary bg-white p-7 shadow-soft" @submit.prevent="login">
      ...
      <div class="mt-6 space-y-4">
        ...
        <label class="block">
          ...
          <input v-model="form.password" class="field mt-1" type="password" autocomplete="current-password" />
        </label>
      </div>
      ...
      <RouterLink class="mt-4 block text-center text-sm font-semibold text-primary" to="/login">
        Student login
      </RouterLink>
    </form>
  </main>
